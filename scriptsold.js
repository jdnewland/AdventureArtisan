...
function generateSimplePrompt() {
    ...
    return {
        prompt: `The party ${goal} ${location} ${obstacle}, aided by ${character}, and then ${twist}.`,
...
function generateWheelOfDestinyPrompt() {
    ...
    const twist = getRandomElement(twists);
    return {
        prompt: `The party, led by ${character1} and ${character2}, ${goal1} and ${goal2} ${location}. They face ${obstacle1} and ${obstacle2}, and then ${twist}.`,
...
function generateBattleOfMindsPrompt() {
    ...
    const twist = getRandomElement(twists);
    return {
        prompt: `The party must navigate the conflict between ${character1} who ${goal1} and ${character2} who ${goal2}. Both are heading towards ${location} where they face ${obstacle}, and then ${twist}.`,
...
function generateFracturedSoulPrompt() {
    ...
    const twist = getRandomElement(twists);
    return {
        prompt: `The party, guided by ${character}, must choose between ${goal1} and ${goal2} ${location}. They face an obstacle ${obstacle} that forces a critical decision, and then ${twist}.`,
